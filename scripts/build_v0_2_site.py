...
SITE_ASSET_VERSION = "20260817-research-context-v1"
...
<main class="page-shell edge-demo" data-accession-demo data-default-accession="GCF_000739105.1">
  ...
  <div class="record-heading edge-search-heading"><div><p class="eyebrow">{local_text('Genome search', '基因组检索')}</p><h1>{local_text('Find transcript 3′-end datasets', '查找转录本 3′ 端数据集')}</h1><p class="record-title">{local_text('Search a reference assembly to see the organism, laboratories, experimental design, publications, raw sequencing and available endpoint files.', '输入参考组装，查看物种、研究单位、实验设计、论文、原始测序数据和可用端点文件。')}</p></div><span class="badge badge-pilot">Beta</span></div>
  ...
  <section data-edge-results hidden>
    <section class="panel organism-profile"><div class="organism-heading"><div><p class="eyebrow">{local_text('Organism and reference', '物种与参考基因组')}</p><h2><em data-edge-scientific-name>—</em> <span data-edge-strain>—</span></h2><p>{local_text('Bacterial transcript 3′-end datasets mapped to one exact reference genome.', '定位到同一个精确参考基因组的细菌转录本 3′ 端数据集。')}</p></div><div class="assembly-actions"><a class="button primary" data-edge-jbrowse href="#">{local_text('Open genome browser', '打开基因组浏览器')}</a><a class="button" data-edge-assembly-page href="#">{local_text('Genome record', '基因组详情')}</a></div></div><dl class="organism-facts"><div><dt>{local_text('Assembly', '参考组装')}</dt><dd data-edge-assembly>—</dd></div><div><dt>{local_text('Chromosome / contig', '染色体 / contig')}</dt><dd data-edge-reference>—</dd></div><div><dt>{local_text('Genome size', '基因组大小')}</dt><dd data-edge-genome-size>—</dd></div><div><dt>{local_text('Replicons', '复制子数量')}</dt><dd data-edge-replicons>—</dd></div><div><dt>{local_text('Source datasets', '来源数据集')}</dt><dd data-edge-tracks>—</dd></div><div><dt>{local_text('3′-end records', '3′ 端记录')}</dt><dd data-edge-records>—</dd></div></dl></section>
    <section class="source-relationship" aria-label="{esc('Source relationship / 来源关系')}"><div><strong>{local_text('Shared raw-data project', '共享原始数据项目')}</strong><p data-edge-relationship>—</p></div><a data-edge-shared-raw href="#" target="_blank" rel="noopener">PRJEB31507 ↗</a></section>
    <section class="edge-studies"><div class="section-heading"><div><p class="eyebrow">{local_text('Study provenance', '研究来源')}</p><h2>{local_text('Who generated the data, and how?', '数据由谁产生，如何测量？')}</h2><p>{local_text('Institution and experiment details are reported per source dataset, because one genome may be represented by several publications.', '研究单位和实验信息按来源数据集分别展示，因为同一基因组可能对应多篇论文。')}</p></div><span class="browser-guide-hint">{local_text('Separate source tracks', '独立来源轨道')}</span></div><div class="study-card-list" data-edge-study-cards></div></section>
