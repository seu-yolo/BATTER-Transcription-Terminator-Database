  ...
  try {
    ...
    return <>
      ...
      {result.data.length === 0 ? <EmptyState>Try a versioned assembly accession or species name.</EmptyState> : <>
        ...
        <div className="table-wrap"><table><thead><tr><th>Assembly</th><th>Organism / strain</th><th>Contigs</th><th>Sources</th><th>Endpoint records</th><th /></tr></thead><tbody>{result.data.map((assembly) => <tr key={assembly.assembly_accession}><td><Link href={`/assemblies/${encodeURIComponent(assembly.assembly_accession)}`}><strong>{assembly.assembly_accession}</strong></Link></td><td>{assembly.organism_name ?? "—"}<br /><span className="muted">{assembly.strain ?? "Strain not recorded"}</span></td><td>{assembly.contigs?.length ?? 0}</td><td>{assembly.source_count}</td><td>{assembly.endpoint_count.toLocaleString()}</td><td><Link href={`/assemblies/${encodeURIComponent(assembly.assembly_accession)}`}>View details</Link></td></tr>)}</tbody></table></div>
