...
export default async function AssemblyDetailPage({ params }: { params: { assemblyId: string } }) {
  ...
  return <>
    ...
    <div className="detail-grid"><div>
      <section className="panel"><h2>Assembly summary</h2><dl className="facts"><div><dt>Assembly</dt><dd>{assembly.assembly_accession}</dd></div><div><dt>Organism</dt><dd>{assembly.organism_name ?? "Not recorded"}</dd></div><div><dt>Strain</dt><dd>{assembly.strain ?? "Not recorded"}</dd></div><div><dt>Contigs</dt><dd>{assembly.contigs.length}</dd></div><div><dt>Source tracks</dt><dd>{assembly.source_count}</dd></div><div><dt>Endpoint records</dt><dd>{assembly.endpoint_count.toLocaleString()}</dd></div></dl></section>
    ...
    </div><aside><section className="panel"><h2>How to use this page</h2><p className="muted">The assembly is the shared coordinate context. Source tracks remain independent because they come from different studies, samples, or evidence definitions.</p><Link className="button primary" href={`/explore?assembly_accession=${encodeURIComponent(assembly.assembly_accession)}`}>Explore this assembly</Link></section></aside></div>
